...
$("#gds-form").on('click', '#addfield', function () {
	...
	if (fieldCount < maxInputs) // max input box allowed
	{
		...
		if(fieldCount == 1) {
			$(".otherWrapper").first().append('<i class="fa fa-trash fa-lg delete" title="Delete" aria-hidden="true" alt="Delete icon" style="font-size: 18px; padding-right: 3px; margin-left: 10px; cursor:pointer">'
					+ '</i></div>');
		}
		...
		$(".otherWrapper").last().after('<div class="otherWrapper" style="margin-bottom: 15px; margin-top: 15px;">'
			...
			+ '<i class="fa fa-trash fa-lg delete" title="Delete"  aria-hidden="true" alt="Delete icon" style="font-size: 18px; padding-right: 3px; margin-left: 13px; cursor:pointer">'
			+ '</i></div>');
